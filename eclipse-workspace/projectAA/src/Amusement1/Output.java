...
public class Output extends Input{
	public static void printouteachTicket() throws IOException {
		...
		output.add(nameofTicket);output.add(nameofage); output.add(amount1);output.add(pricestring);output.add(Discountname);		
	}
	public static void Totalprice() throws IOException{
		System.out.println("================에버랜드=================");
		for(int i = 0;i<output.size();i+=6) {
			System.out.printf("%s %s X %s %s원 *%s\n", output.get(i+1), output.get(i+2), output.get(i+3), output.get(i+4), output.get(i+5));
			total_price += Integer.parseInt(output.get(i+4));
		}
		System.out.println();
